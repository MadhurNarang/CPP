...
void DRAW :: LINE_HOR(int columnl, int column2, int row, char c) 
{ 
    for ( columnl; column1<=column2; column1++) 
     { 
           gotoxy(column1,row) ; 
           cout <<c ; 
      }
...
{ 
   ...
   char vno[6] ; 
   gotoxy(5,5) ;
  cout ""Enter Vote no. or Press <ENTER> to exit 1 " ;
 gets(vno) ; 
if (strlen(vno) == 0) 
           return ; 
int found=0, count=0;
...
do 
{
                ...
                f.cpen("VOTER.DAT", ios::in);
               while (f.read((char *) this, sizeof(VOTER))) 
                  {
                      count++ ; 
                     if (lstrcmpi(vno,voterno)) 
                           {
                               found = 1 ;
                              break ;
                           }
...
{ 
      ...
      temp.open("temp.dat", ios::out) ;
      int found=0 ; 
     while (file.read((char I") this, sizeof(VOTER))) 
         {
                if (Istrcmpi(voterno,vno))
                     found = 1 ;
              else 
                     temp.write((char *) this, sizeof(VOTER)) ; 
       }
...
//********************************************************** 
void CANDIDATE :: CANDIDATE_INFORMAT|0N(void) { 
clrscr() ; 
int row=5, found=0 ; 
char ch; 
gotoxy(27,1) ; 
...
void CANDIDATE :: ADD_CANDIDATE(void) 
{
       ...
       do 
             {
                   ...
                   while (f.read((char *) this, sizeof(CANDlDATE))) 
                          count++ ; 
                 f.c|ose() ;
                sno=count; 
               if (count >= 15) 
 ...
 if (ch == 'y' | 1 ch == 'v') 
     {
            fstream f ; 
            f.open("CAN.DA ", ios::app) ;
            f.write((char *)this, sizeof(CANDIDATE)) ;
            f.c|ose() ; 
    } 
       gotoxy(10,18) ; 
     cout <<"Add more? (y/n) ; " ; ch = getche() ;
 }whi|e (c == 'y' II ch == 'Y'); 
//********************************************************** 
...
void CANDIDATE :: RESET_STATUS(void) 
     {
         ...
         while (!file.eof()) 
         {
               file.read((char *) this, sizeof(CAND|DATE)) ;
               if (file.eof())
                     break ; 
               votes = 0 ;
               temp.write((char *) this, sizeof(CAND|DATE)) ;
        } 
file.close() ; 
temp.close() ;
 file.cpen("CAN.DA ", ios::out) ;
 temp.open("temp.dat", ios::in) ;
 temp.seekg(0,ios::beg) ; 
while (!temp.eof()) 
   {
          temp.read((char *) this, sizeof(CAND|DATE)) ; 
          if (temp.eof()) 
                  break ;
        flle.write(( char *) thls, slzeof(CANDlDATE)) ;
 }
 file.close() ; 
temp.close() ; 
   ...
   while (!temp.eof()) ; 
       {
             ...
             if (temp.eof()) 
                  break ; 
             file.write((char *) this, sizeof(CAND|DATE)) ;
     } 
   file.close() ; 
   temp.c|ose() ; 
  return 1; 
...
int CANDIDATE :: DELETE_RECORD(int sno)
     {  
          ...
          temp.cpen("temp.dat", ios::out) ;
           int found=0, count=0 ; 
          while (fIleJeaduchar ') this, sizeof(CANDlDATE))) 
               { 
                      count++ ; 
                     if (sno == count) 
                            found = 1 ;
                    else
                           temp.write((char *) this, sizeof(CANDlDATE)) ; 
             }
 file.close() ; 
temp.close() ;
 if (!found) 
...
temp.open("temp.dat", ios::in) ; 
while (temp.read((char *) this, sizeof(CANDlDATE))) 
...
//****************************************************** 
int  VOTING__DATE :: VALID_DATE(void) 
{ 
       fstream f ; 
       f.open("ELEDATE.DA T", ios::in) ; 
       f.read((char *) this, sizeof(VOTING_DATE)) ; 
       f.close() ; 
...
//********************************************************** 
void VOT!NG_DATE :: SET_DATE(void)
 {
     clrscr() ;
    ...
    cin >>d2 >>m2 >>y2 ; 
    if ((y2<y1) l | (y2==y1 && m2<m1) | | (y2==y1 && m2==m1 && d2<d1)) 
        {
           gotoxy(5,20) ;
           cout <<"Invalid Date" ;
           getch(); 
